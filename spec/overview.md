...
```
主頁
  ├── BlockB：今天整體狀況如何？（懶人摘要 + 待驗證清單 + 延續性追蹤）
...
| 區塊 | 內容 | 資料來源 |
|------|------|----------|
| 盤後懶人摘要 | 用白話說明今天市場發生什麼事，不重述完整數字表 | `buildMarketBrief().lazySummary` |
| 市場環境 | regime label badge + 描述文字 | `buildMarketBrief()` |
| 待驗證清單 | 不是推薦股，而是列出隔日需要驗證的族群/結構狀態 | `buildMarketBrief().validationItems` |
| 延續性追蹤 | 昨日強勢是否續強、資金是否撤退、廣度是否擴散 | `buildMarketBrief().continuityItems` |
...
- 優先放入結構分歧項目，例如「資金入但走勢弱」、「族群漲但法人賣」
- 補入強勢族群，說明隔日要驗證是否擴散到更多族群內個股
- 補入資金流入 Top1，說明需驗證是單日集中或可延續成族群動能
- 文案禁止使用推薦、買進、進場等操作語言

**延續性追蹤規則**：

- 強勢族群：追蹤法人連買、廣度是否維持、族群內個股是否擴散
- 分歧族群：追蹤價格、資金、廣度是否開始同步
- 弱勢族群：追蹤法人是否持續流出、資金是否撤退
- 目前 V1 以 `institutionalStreak`、`category`、`breadth` 作為延續性代理；真實 API 接入後應改為明確比對 T-1 / T / 區間序列
...
**3. 純函式集中，可測試**
`buildMarketBrief()`、`buildStockChecks()` 等核心邏輯都是純函式，有 Vitest 覆蓋。BlockB 的懶人摘要、待驗證清單、延續性追蹤都由同一個規則引擎輸出。
...
**8. 進階入口層尚未實作**
全局搜尋列、懶人 / 專業模式切換、PRO badge 等仍未動工。既有 BlockB 已先提供白話懶人摘要、待驗證清單與延續性追蹤。
...
| 延續性追蹤歷史序列 | V1 用 streak/category/breadth 代理，後端需提供 T-1/T/區間序列做精準比對 |
